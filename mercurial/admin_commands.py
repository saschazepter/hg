...
if typing.TYPE_CHECKING:
    ...
    # TODO define a protocol instead of using Any
...
@command(
    ...
    [
        ...
        (
            b'',
            b'override-reshape-and-reshard-check',
            None,
...
def _quote_names(names: list[bytes]) -> bytes:
    return b", ".join(b'"%s"' % name for name in names)
...
def _validate_shape_update(old_store_shards, new_store_shards, **opts) -> None:
    ...
    if not opts.get("override_reshape_and_reshard_check"):
        ...
        if shapes and shards:
            msg = _(
                b"cannot reshape and reshard in the same update\n"
                b"shapes changed: %s\n"
                b"shards changed: %s"
            )
            hint = _(
                b"see 'hg help \"narrow.updating the shapes config\"' for"
                b" how to split this in two updates"
            )
            raise error.InputError(
                msg % (_quote_names(shapes), _quote_names(shards)), hint=hint
            )
...
def _shape_update(ui: UiT, repo: RepoT, **opts) -> int:
    ...
    with repo.store_shapes_lock(wait=False):
        ...
        _validate_shape_update(old_store_shards, new_store_shards, **opts)

        header = (
            b'# Do not edit this file by hand!\n'
            b'# Use `hg admin::narrow-server --shape-update` instead,\n'
            b'# which will ensure the update is made safely.\n'
        )
        if not shapes.startswith(header):
