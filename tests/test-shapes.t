...
Update from file, adding a new shape that depends on the existing shard
  ...
  # Use `hg admin::narrow-server --shape-update` instead,
  # which will ensure the update is made safely.
  version = 0
  [[shards]]
  name = "foo"
  paths = ["foo"]
...
-----------------------------------
  ...
  $ cp .hg/store/server-shapes ../concurrent-update
  $ cat >> ../concurrent-update <<EOF
  > [[shards]]
  > name = "concurrent"
  > requires = ["foo"]
  > shape = true
...
The shapes lock is held for the whole update, so a second update fails
  ...
  $ hg admin::narrow-server --shape-update -f ../concurrent-update
...
The lock is released afterwards, so now another update can now go through
  ...
  $ hg admin::narrow-server --shape-update -f ../concurrent-update
  $ hg admin::narrow-server --shape-fingerprints
  f182ace793f1c0e450c0067f66fe84b1ccec6fa61ffc0eb48f6fe311d2f7b071 concurrent
  00dfe7451b0897c077166f360d431a57ea09a5279863b00cfe9d60cefa657dea full
