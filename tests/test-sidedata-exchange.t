...
what the server is asking
  ...
  abort: cannot push: required sidedata category not supported by this client: 'test-1-2'
  [255]
...
Add the required capabilities
  ...
  > testsidedata2=$TESTDIR/testlib/ext-sidedata-2.py
  > EOF
  $ cat ../source-repo/.hg/hgrc
  [extensions]
  testsidedata2=$TESTDIR/testlib/ext-sidedata-2.py

We expect the target to have sidedata that it generated on-the-fly during pull
  $ hg pull -r . ssh://user@dummy/source-repo  --traceback
  pulling from ssh://user@dummy/source-repo
  adding changesets
  adding manifests
  adding file changes
  added 3 changesets with 3 changes to 2 files
  ...
  (run 'hg update' to get a working copy)
  $ hg debugsidedata -m 0 --traceback
  2 sidedata entries
   entry-0001 size 4
   entry-0002 size 32
  $ hg debugsidedata -m 1 -v --traceback
  2 sidedata entries
   entry-0001 size 4
    '\x00\x00\x00V'
   entry-0002 size 32
