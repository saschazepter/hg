...
local:
	...
	if test -e .local-venv/$(PYBINDIRNAME)/rhg; then ln -s -f .local-venv/$(PYBINDIRNAME)/rhg rhg-local; fi
...
cargo-clippy:
	cd $(HGROOT)/rust \
		...
		&& $(CARGO) clippy --workspace --all-targets --features "full-tracing" -- -D warnings
